...
export const saveMaxCardsCountAC = (maxCardsCount: number) => ({
	type: 'packs/SAVE-MAX-CARDS-COUNT',
	maxCardsCount
} as const)
export const saveUserIdAC = (userId: string) => ({type: 'packs/SAVE-USER-ID', userId} as const)
export const searchPackNameAC = (searchPackName: string) => ({
	type: 'packs/SAVE-SEARCH-PACK-NAME',
	searchPackName
} as const)
